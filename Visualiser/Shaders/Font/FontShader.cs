...
namespace Visualiser.Shaders.Font
{
	public class FontShader : IDisposable
	{
		...
		public Buffer ConstantPixelBuffer { get; set; }
		...
		private bool InitializeShader(Device device, IntPtr windowHandler, string vsFileName, string psFileName)
		{
			try
			{
				...
				// Setup full paths
				vsFileName = SystemConfiguration.FontFilePath + vsFileName;
				psFileName = SystemConfiguration.FontFilePath + psFileName;
				...
				{
					Usage = ResourceUsage.Dynamic,
					SizeInBytes = SharpDX.Utilities.SizeOf<PixelBuffer>(),
					BindFlags = BindFlags.ConstantBuffer,
					CpuAccessFlags = CpuAccessFlags.Write,
					OptionFlags = ResourceOptionFlags.None,
					StructureByteStride = 0
